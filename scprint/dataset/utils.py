...
def validate(adata, lb):
    ...
    if not lb.DevelopmentalStage.validate(
        adata.obs["development_stage_ontology_term_id"], field="ontology_id"
    ).all():
    ...
    if not lb.Gene.validate(adata.var.index, field="ensembl_gene_id").all():
...
def load_dataset_local(
    remote_dataset, folder, name, description, use_cache=True, only=None
):
    saved_files = []
    ...
    for file in files:
        ...
        #    continue
        # if organism[0] == "NCBITaxon:10090":
        #    print("mouse")
        #    continue
        file.save()
        # if location already has a file, don't save again
        if use_cache and os.path.exists(os.path.expanduser(folder + file.key)):
            print(f"File {file.key} already exists in storage")
        else:
            file.path.download_to(folder + file.key)
...
def populate_my_ontology(
    ...
    # genes
    names = bt.Gene().df()["ensembl_gene_id"]
...
def get_molecular_embeddings(
    genes_ids,
    tool_path=__file__ + "/../tools/",
):
    """
    Get the molecular embeddings for all genes in the dataset.
    """
