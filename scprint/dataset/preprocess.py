...
class Preprocessor:
    ...
    def __call__(
        ...
        """
        files = []
        for i, file in enumerate(ln_dataset.files.all()[:1]):
            ...
            if prevsize / adata.shape[0] > self.maxdropamount:
                raise Exception(
                    "Dataset dropped due to low expressed genes and unexpressed cells"
                )
            ...
            intersect_genes = set(adata.var.index).intersection(set(self.genesdf.index))
            ...
            unseen = set(self.genesdf.index) - set(adata.var.index)
            ...
            adata.uns["unseen_genes"] = unseen
            data_utils.validate(adata, self.lb)
            # length normalization
            if adata.obs["assay_ontology_term_id"].isin(FULL_LENGTH_ASSAYS).any():
            ...
            # QC
            adata.var[self.genesdf.columns] = self.genesdf.loc[adata.var.index]
            ...
            if self.erase_prev_dataset:
                ...
                file = ln.File(
                    adata, is_new_version_of=file, description="preprocessed by scprint"
                )
            file.save()
            files.append(file)
