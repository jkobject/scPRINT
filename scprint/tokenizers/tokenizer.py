...
class GeneVocab(Vocab):
    ...
    def __init__(
        self,
        df_list_or_vocab: Union[List[str], Vocab, pd.DataFrame],
        ...
        """
        if isinstance(df_list_or_vocab, Vocab):
            _vocab = df_list_or_vocab
            if specials is not None:
                raise ValueError(
                    "receive non-empty specials when init from a Vocab object."
                )
        elif isinstance(df_list_or_vocab, pd.DataFrame):
            _vocab = self._build_vocab_from_iterator(
                df_list_or_vocab.index.tolist() + [default_token],
                specials=specials,
                special_first=special_first,
            )
        elif isinstance(df_list_or_vocab, list):
            _vocab = self._build_vocab_from_iterator(
                df_list_or_vocab + [default_token],
                specials=specials,
                special_first=special_first,
            )
        else:
            raise ValueError(
                "df_list_or_vocab must be a list of gene names or a Vocab object or a dataframe."
            )
...
def tokenize_and_pad_minibatch(
    expr: np.ndarray,
    token_names: np.ndarray,
    max_len: int,
    vocab: Vocab,
    cls_tokens: list[str] = ["timepoint", "cell"],
    return_pt: bool = True,
    ...
    if expr.shape[1] != len(gene_ids):
        raise ValueError(
            f"Number of features in expr ({expr.shape[1]}) does not match "
    ...
    for i in range(len(expr)):
        ...
        if include_zero_gene:
            ...
            values = row[idx]
            genes = gene_ids[idx]
        genes = np.insert(genes, 0, cls_tokens)
        values = np.insert(values, 0, [-1] * len(cls_tokens))
