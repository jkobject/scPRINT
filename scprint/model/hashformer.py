...
from .hsparse_attn import attention_fn
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.hashes_per_head = hashes_per_head
        ...
        if not self.flash:
            ...
            ), "sequence_length must be specified for non-flash attention"
            print(
    ...
    def forward(self, x):
        ...
        # calculate query, values for all heads in batch and move head forward to be the batch dim
        q, v = self.c_attn(x).split(self.n_embd, dim=2)

        q = (
            q.view(B, T, self.n_head, C // self.n_head).transpose(1, 2).contiguous()
        )  # (B, nh, T, hs)
        v = (
            v.view(B, T, self.n_head, C // self.n_head).transpose(1, 2).contiguous()
        )  # (B, nh, T, hs)

        k = F.normalize(q, p=2, dim=-1).type(torch.bfloat16)
        ...
        y = attention_fn(
            ...
            hashes_per_head=self.hashes_per_head,
...
class MLP(nn.Module):
    def __init__(self, n_embd, bias, dropout, scaling=4):
        super().__init__()
        self.c_fc = nn.Linear(n_embd, scaling * n_embd, bias=bias)
        self.c_proj = nn.Linear(scaling * n_embd, n_embd, bias=bias)
            ...
            [
                nn.ModuleList(
                    [
                        ...
                        LayerNorm(n_embd, bias=bias),
                        MLP(n_embd, bias, dropout, scaling=ff_scaling),
                    ]
                )
...
class Block(nn.Module):
    def __init__(self, config):
        ...
        self.attn = CausalSelfAttention(
            ...
            config.bias,
            config.nb_hash,
            config.hashes_per_head,
            config.dropout,
            config.sequence_length,
        )
        self.ln_2 = LayerNorm(config.n_embd, bias=config.bias)
        self.mlp = MLP(config.n_embd, config.bias, config.dropout)
...
class FullHashformer(nn.Module):
    def __init__(self, config):
        super().__init__()
