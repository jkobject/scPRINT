...
class Collator:
    def __init__(
        ...
        self.how = how
        self.organism_ids = (
            set(self.org_to_id.values())
            if self.org_to_id is not None
        ...
        for organism in set(self.genedf.organism):
            ...
            org = self.org_to_id[organism] if self.org_to_id is not None else organism
            self.start_idx.update(
                {org: np.where(self.genedf.organism == organism)[0][0]}
            )
            if len(self.valid_genes) > 0:
                self.accepted_genes.update({org: ogenedf.index.isin(self.valid_genes)})
