...
class MyCLI(LightningCLI):
    ...
    def _add_subcommands(self, parser, **kwargs) -> None:
        ...
        for subcommand in TASKS:
            ...
            parser.add_argument("--output_filename", type=str, help=(
            ...
            added = parser.add_method_arguments(subcommand[1], "__call__", skip=set(['model', "adata", "dataloaders", "datamodule"]))
    ...
    def _run_subcommand(self, subcommand: str) -> None:
        ...
        if subcommand in self.subcommands():
            ...
            adata.obs['organism_ontology_term_id'] = self.config_init[subcommand]["species"]
            preprocessor = Preprocessor(do_postp=False, force_preprocess=True, skip_validate=True, use_layer="counts")
            adata = preprocessor(adata)
            conf = dict(self.config_init[subcommand])
            model = scPrint.load_from_checkpoint(
                self.config_init[subcommand]['ckpt_path'],
                precpt_gene_emb=None
            )
            for key in ["seed_everything", "config", "species", "cache", "ckpt_path", "adata", "output_filename"]:
                conf.pop(key, None)
            ...
            if subcommand == "embed":
                ...
                print(metrics)
                print()
                print("saving the file under the path: ", self.config_init[subcommand]['output_filename'])
                adata.write(self.config_init[subcommand]['output_filename']+ ".h5ad")
            ...
            if subcommand == "gninfer":
                ...
                adata = gn(
                    model=model,
                    adata=adata,
                    cache=self.config_init[subcommand]['cell_type'],
                )
                print("saving the file under the path: ", self.config_init[subcommand]['output_filename'])
                adata.write(self.config_init[subcommand]['output_filename']+ "_"+self.config_init[subcommand]['cell_type']+"_grn.h5ad")
            ...
            if subcommand == "denoise":
                ...
                i = 0
                adata.layers['denoised'] = adata.X.tolil()
                elems = random_indices if random_indices is not None else range(adata.shape[0])
                for idx in elems[:dn.plot_corr_size]:
                    adata.X[idx, adata.var.index.get_indexer(np.array(model.genes)[genes.cpu().numpy()[i]])] = expr_pred[i].cpu().numpy()
                    i += 1
                adata.X = adata.X.tocsr()
                print("saving the file under the path: ", self.config_init[subcommand]['output_filename'])
                adata = sc.read_h5ad(self.config_init[subcommand]['adata']+ "_denoised.h5ad")
